...
@section('title')
    <h4>
        ...
        <span class="font-weight-semibold">Main</span> - Tambah Ambulan
    </h4>
@endsection
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form class="form-horizontal" action="{{route('ambulan.store')}}" method="post" enctype="multipart/form-data" files=true>
                @csrf
                <fieldset class="mb-3">
                        ...
                        <div class="col-lg-10">
                            <select name="instansi" id="instansi" data-placeholder="Pilih Instansi" class="form-control select" data-fouc>
                                ...
                                @foreach (\App\Instansi::get()->sortBy('nama') as $value)
                                    <option value="{{$value->instansi_id}}" {{ (collect(old('instansi'))->contains($value->instansi_id)) ? 'selected':'' }}>{{$value->nama}}</option>
                                @endforeach
                            </select>
